...
//       error += norm(itpp::eye_c(size_space)-U); 
    ...
    // PArece que en todas las dimensiones funciona 
    //  ./test_spins -o test_horizontal_proyector --i1 3 --i2 3 --i3 5
    int nv=i2.getValue(), nh=i3.getValue(); 
    int q= nv*nh; int d=pow_2(q);
...
//     itpp::cvec state_l, state_h, state_r, state, prestate;
//     double error=0.;
// clear && make -B test_spins2d && ./test_spins2d -o test_create_small_base_2d --i2 2 --i3 3 
...
//     nv=2, nh=1; 
    double error=0.;
    itpp::Mat<itpp::Array<CompactSymmetric2DBaseMember> > All2DStates;
    itpp::Array<CompactSymmetricBaseMember> basis_states_horizontal;
    build_rotationally_symmetric_2D_base(nh, nv, All2DStates, basis_states_horizontal);
    // normalization of each state {{{
    {
    int kv=0, kh=0;
    itpp::cvec state_l, state_r;
    itpp::Array<CompactSymmetric2DBaseMember> States_Sector=All2DStates(kv, kh);
    int size_space= States_Sector.size();
    for (int i=0; i<size_space; i++){
      state_l=DecodeCompactSymmetric2DBaseMember(States_Sector(i));
      cout << state_l << endl; 
      cout << "Norma state_l=" << norm(state_l) << endl;
      error += abs(norm(state_l)-1); 
    }
    cout << "Error 1 is " << error << endl;
    abort();
    }
    // }}}
    // orthonormality in each sector {{{
    ...
    U=0.;
      for (int i=0; i<size_space; i++){
        ...
        //       abort(); 
        for (int j=0; j<size_space; j++){
          state_r=DecodeCompactSymmetric2DBaseMember(States_Sector(j));
          //         cout << "norma(state_r)=" << norm(state_r) << endl;
          //         if (j==1) abort(); 
          U(i,j) =  itpp::dot(conj(state_l),state_r) ;
          //         cout << itpp::dot(conj(state_l),state_r) << endl;
      ...
      error += norm(itpp::eye_c(size_space)-U); 
    // }}}
    cout << "Error en test_create_small_base_2d es " << error << endl;
    //}}}
...
#else // INCLUDECUDA
    cerr << "Option not available. Compiled without CUDA suport\n" ;
    abort();
#endif // INCLUDECUDA
    //}}}
  } else {// {{{
